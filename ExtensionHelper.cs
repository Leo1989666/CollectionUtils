...
public static class ExtensionHelper
{
    ...
    public static void Optimize(this ParticleSystem particleSystem)
    {
        ParticleSystem[] particleArr = particleSystem.gameObject.GetComponentsInChildren<ParticleSystem>();
        int length = particleArr.Length;
        for (int i = 0; i < length; i++)
        {
            string name = particleArr[i].name;
            char charNumber = name[name.Length - 1];
            int number;
            if (int.TryParse(charNumber.ToString(), out number))
            {
                int level = (int)SettingQualityController.CurrentQualityLevel;
                particleArr[i].gameObject.SetSafeActive((level >= number));
            }
        }
    }
}

public static class PineUtil
{
    ...
    public static bool CheckLineIntersectRectangle(float x0, float y0, float x1, float y1, float xmin, float ymin, float xmax, float ymax)
    {
        ...
        int outcode0 = PineUtil.ComputeOutCode(x0, y0, xmin, ymin, xmax, ymax);
        int outcode1 = PineUtil.ComputeOutCode(x1, y1, xmin, ymin, xmax, ymax);
        bool accept = false;
        while (true)
        {
            ...
            else
            {
                ...
                if (outcodeOut == outcode0)
                {
                    x0 = x;
                    y0 = y;
                    outcode0 = PineUtil.ComputeOutCode(x0, y0, xmin, ymin, xmax, ymax);
                }
                else
                {
                    x1 = x;
                    y1 = y;
                    outcode1 = PineUtil.ComputeOutCode(x1, y1, xmin, ymin, xmax, ymax);
                }
            }
        }
        return accept;
    ...
    /// <param name="TFSRotation"></param>
    /// <param name="TSFRotation"></param>
    /// <returns></returns>
    ...
    public static string GetTimeZoneAnalytic()
    {
        ...
        string hourStr = hour > 9 ? hour.ToString() : PineUtil.CreateStrings("0", hour.ToString());
        string minStr = minute > 9 ? minute.ToString() : PineUtil.CreateStrings("0", minute.ToString());

        return PineUtil.CreateStrings(hourStr, "_", minStr);
    }
